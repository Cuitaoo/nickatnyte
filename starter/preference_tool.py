...
def _is_attribute_only_override(value: str) -> bool:
    tokens = set(TOKEN_RE.findall(normalize_value(value)))
    if not tokens:
        return False
    if len(tokens) <= 3 and tokens <= set(MATERIALS) | set(COLORS):
        return True
    if re.fullmatch(r"(?:under|below|less than|up to)\s*\$?\s*\d+(?:\.\d{1,2})?", value):
        return True
    return False


def _override_removals(values: list[PreferenceValue]) -> list[PreferenceRemoval]:
    attributes = {
        item.attribute
        for item in values
        if item.attribute in {"material", "color", "budget"}
    }
    return [PreferenceRemoval(attribute=attribute) for attribute in sorted(attributes)]
...
def parse_preference_fallback(
    ...
    if override:
        override_target = _first_match(
            r"(?:what i need is|instead|changed my mind(?:,|:)?(?: i need)?)\s*[:,-]?\s*(.+?)(?=[.;]|$)",
            lowered,
        )
    reset_product_preferences = override and not (
        override_target and _is_attribute_only_override(override_target)
    )
    if reset_product_preferences:
        extracted_category = _first_match(
            r"(?:what i need is|instead|changed my mind(?:,|:)?(?: i need)?)\s*[:,-]?\s*(.+?)(?=[.;]|$)",
            lowered,
    ...
    removals = _override_removals(values) if override and not reset_product_preferences else []
