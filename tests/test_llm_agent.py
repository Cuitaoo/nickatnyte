...
class PreferenceInterpreterTest(unittest.TestCase):
    ...
    def test_missing_api_key_does_not_construct_a_client(self) -> None:
        with patch.dict(
            os.environ,
            {"OPENAI_ENABLED": "true", "OPENAI_API_KEY": "", "OPENAI_BASE_URL": ""},
            clear=True,
        ):
            self.assertIsNone(PreferenceInterpreter.from_environment())

    def test_local_base_url_uses_empty_api_key(self) -> None:
        with patch("starter.llm_agent.ChatOpenAI") as chat:
            chat.return_value = FakeChatModel()
            with patch.dict(
                os.environ,
                {
                    "OPENAI_ENABLED": "true",
                    ...
                    "OPENAI_MODEL": "Qwen/Qwen3-4B-Instruct",
                },
                clear=True,
            ):
                interpreter = PreferenceInterpreter.from_environment()

        self.assertIsInstance(interpreter, PreferenceInterpreter)
        _, kwargs = chat.call_args
        self.assertEqual(kwargs["api_key"], "EMPTY")
        self.assertEqual(kwargs["base_url"], "http://localhost:8000/v1")
        self.assertEqual(kwargs["model"], "Qwen/Qwen3-4B-Instruct")
